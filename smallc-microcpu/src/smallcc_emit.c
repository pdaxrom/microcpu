...
extern int
  *stage, litlab, litptr, csp, output, oldseg, usexpr, objectmode, backend,
  pubclass, pcode_argc,
  *snext, *stail, *slast;
...
*/
int header()  {
  if(backend == BACKEND_PCODE) return pcode_header();
  oldseg = 0;
...
*/
int trailer()  {  
  if(backend == BACKEND_PCODE) return pcode_trailer();
  if(objectmode) {
    outline("");
    outline("align 1");
    cptr = STARTGLB;
...
int public(ident) int ident;{
  if(backend == BACKEND_PCODE) return pcode_public(ident);
...
int external(name, size, ident) char *name; int size, ident; {
  if(backend == BACKEND_PCODE) return pcode_external(name, size, ident);
  if(objectmode) {
    outstr("extern ");
    outname(name);
    newline();
...
int dumplits(size) int size; {
  int j, k;
  if(backend == BACKEND_PCODE) return pcode_dumplits(size);
  k = 0;
  while (k < litptr) {
...
int dumpzero(size, count) int size, count; {
  if(backend == BACKEND_PCODE) return pcode_dumpzero(size, count);
  if(count > 0) {
    if(size == 1)
         gen(BYTEr0, count);
    else if(size == BPW)
...
int outcode(pcode, value) int pcode, value; {
  int part, skip, count;
  char *cp, *back;
  if(backend == BACKEND_PCODE) return pcode_outcode(pcode, value);
  part = back = 0;
  skip = NO;
