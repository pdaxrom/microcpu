...
int test(label, parens)  int label, parens;  {
  int is[7];
  int *before, *start;
  if(parens) need("(");
  while(1) {
...
*/
int zerojump(oper, label, is) int oper, label, is[]; {
...
int level3 (is) int is[]; {return skim("||", EQ10f, 1, 0, level4,  is);}
int level4 (is) int is[]; {return skim("&&", NE10f, 0, 1, level5,  is);}
int level5 (is) int is[]; {return down("|",            0, level6,  is);}
int level6 (is) int is[]; {return down("^",            1, level7,  is);}
int level7 (is) int is[]; {return down("&",            2, level8,  is);}
int level8 (is) int is[]; {return down("== !=",        3, level9,  is);}
int level9 (is) int is[]; {return down("<= >= < >",    5, level10, is);}
int level10(is) int is[]; {return down(">> <<",        9, level11, is);}
int level11(is) int is[]; {return down("+ -",         11, level12, is);}
...
int primary(is)  int *is; {
  ...
  memset(is, 0, 7 * sizeof(*is));    /* clear host-sized "is" array */
...
int stowlit(value, size) int value, size; {
...
*/
int skim(opstr, tcode, dropval, endval, level, is)
  char *opstr;
...
*/
int down1(level, is) int (*level)(), is[]; {
  int k, *before, *start;
  setstage(&before, &start);
  k = (*level)(is);
...
*/
int down2(oper, oper2, level, is, is2)
...
*/
int calc(left, oper, right) int left, oper, right; {
  switch(oper) {
...
*/
int calc2(left, oper, right) unsigned left, right; int oper; {
  switch(oper) {
