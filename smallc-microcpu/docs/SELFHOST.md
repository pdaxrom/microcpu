...
`pcode-tests/001..049`, including comparisons, short-circuit logical
...
- `smallcc`: all modules now generate p-code.  Estimated p-code image is about
  64.0 KB versus 187.3 KB summed native object size, roughly 123.3 KB smaller.
...
- `smallcc`: direct p-code object link smoke passes with `PCODE_OPT=1`.  The
  ...
  about 61.5 KB, below the 64K binary limit.
...
- `smallcc`: links and starts on `hc1200-cpu`; it prints the banner through
  UART, then halts with `V0=0xca10`.  The current direct-linked image is 62,422
  bytes.  The hosted diagnostic identifies the failed allocation as the
  8,192-byte literal pool (`litq`), with 190 bytes left below the hosted heap
  guard.
