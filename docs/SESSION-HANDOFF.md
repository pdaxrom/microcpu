...
**Поправка 2026-08-29:** в исходной передаче была пропущена утечка новых
native SD-служб в guest. В `a985039` фильтровались только F000..F007, не
F008..F00F. Текущий ASM исправлен; размер SD+FIS — **3504 code + 64 context +
16 free**, без изменения ISA/RTL-логики. Прежняя прошитая база и её хеши ниже
относятся именно к `a985039`, не к исправленному образу. Актуальные проверки
и статус новой сборки: [изоляция native-служб](native-service-isolation.md).
Исправление закоммичено как **45a1b6859922874575888f7346c85b1c7d01dd5a**.
Полная RT-11 симуляция (8/8), 4040 FIS cases и регрессия прошли. Diamond
**3.14.0.75.2** собрал новый JED **60FA**, который уже прошит с успешным
FLASH Verify. Точная копия на обоих hosts:
`boards/hc1200-microcomp/impl1-sdboot/native-isolation-45a1b68/microcomp_impl1.jed`;
SHA-256 `d878e1927fa3996499b4ae391df12fa6897f8ed70d1d33fe52fa96bce7206879`.
Ресурсы/timing прежние: 1095 LUT4, 7 EBR, 37.627 MHz. UART B не отключали;
после прошивки пользователь подтвердил физическую загрузку RT-11FB V05.03 с
DM0 и прислал `SHOW CONFIGURATION`: PDP 11/73A, 56KB, Floating Point
Microcode, EIS, FIS, Cache Memory и 50 Cycle System Clock. Это аппаратное
подтверждение текущего образа `45a1b68` / `60FA`. Затем `.DIR *.OBJ` на плате
вывел 4 файла, 78 блоков и 51455 свободных блоков, то есть ключевой аппаратный
сценарий SHOW/DIRECTORY также пройден.
Раздел 4 ниже теперь является **исторической recovery-базой a985039**, а не
описанием текущего содержимого FPGA. Точные новые логи:
[acceptance 45a1b68](hc1200-sd-diamond.md).
...
## 4. Предыдущая стабильная сборка a985039, сохранённая для восстановления
...
Raw native service window `0xF000..0xF00F` должен быть закрыт от guest.
В старой базе `a985039` это требование выполнялось только для F000..F007:
гость мог обращаться к F008/F00A/F00C. Это воспроизведённый дефект, а не
допустимый гостевой API. Текущий ASM закрывает полный 16-byte диапазон,
отдельно сохраняя точный 8-byte DL11 decode; см. поправку в начале файла.
...
изолированный Diamond. Не расходовать последние 16 code words незаметно и
не изменять stable binary под тем же именем. После исправления изоляции
свободны 16 слов (19 относились к a985039). Разделять «собрано», «sim pass»,
...
build evidence. Последний выполненный шаг — **изоляция native-служб в ASM,
полная RT-11/regression проверка, Diamond и прошивка 45a1b68 / JED 60FA**.
Пользователь подтвердил вывод нового RT-11 boot, `SHOW CONFIGURATION` и
`.DIR *.OBJ` через оставленный открытым UART-терминал; параллельного чтения
порта не было.
Не повторять FLASH программирование ради восстановления контекста; сначала
проверь последнее сообщение пользователя. Новую сессию или автоматизацию
этот документ сам не создаёт.
